...
window.handleImageFileSelected = async function(files) {
  ...
  try {
    ...
    // First attempt: try our internal Next.js API route if available
    try {
      const internalFormData = new FormData();
      internalFormData.append('image', file);
      const internalRes = await fetch('/api/upload-image', {
        method: 'POST',
        body: internalFormData,
      });
      if (internalRes.ok) {
        const internalData = await internalRes.json();
        if (internalData.success && internalData.url) {
          directUrl = internalData.url;
        }
      }
    } catch (_) {
      // Fall back to direct API if internal route is unavailable
    }

    // Second attempt: direct upload using configured key
    if (!directUrl) {
    ...
    if (directUrl) {
      ...
      throw new Error('ছবি আপলোড সম্পন্ন হয়নি');
    }
  } catch (error) {
