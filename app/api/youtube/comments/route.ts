...
import { getVideoComments } from '@/lib/youtube/service';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateCheck = checkRateLimit(ip, '/api/youtube/comments');
    ...
    if (parsed.type !== 'VIDEO') {
      return NextResponse.json(
        {
          error:
            'The Comment Viewer requires a specific YouTube video link (e.g. youtube.com/watch?v=... or youtu.be/...) rather than a channel link.',
        },
        { status: 400 }
      );
    }

    const result = await getVideoComments(parsed.id, order, pageToken);
    ...
    console.error('API Error in /api/youtube/comments:', err);
    return NextResponse.json(
      {
        error:
          err instanceof Error
            ? err.message
            : 'Unable to retrieve comments for this video at this time.',
      },
      { status: 500 }
    );
  }
